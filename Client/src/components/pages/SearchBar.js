...
const SearchBar = () => {
  ...
  useEffect(() => {
    // Fetch pickup options from an API
    const fetchPickupOptions = async () => {
      try {
        const response = await axios.get("YOUR_PICKUP_OPTIONS_API_ENDPOINT");
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(
        `YOUR_SEARCH_API_ENDPOINT?query=${searchTerm}&pickup=${selectedOption}`
      );
